...
* **本局统计** — 概览、**每种打法的听牌与番数**（含够不够起胡番、每张听牌还剩几张）、
  四家副露与牌河、**34 种牌的剩余张数追踪**、本场逐局战绩。
...
界面带一层动态特效：背景是缓慢漂移的多色极光；标题有流光扫过；按钮划过高光；
中央信息牌绕着一圈旋转的金色光环；轮到谁出牌，谁的名牌会呼吸式发光；
发牌时手牌依次落下，别人打出的牌会弹跳落位；可以吃碰杠时按钮持续脉动；
自己和牌会撒彩纸。

所有动画只改 `transform` / `opacity` / `background-position`，不触发重排，
即使满屏九十多张牌也不掉帧。系统开启「减弱动态效果」时会自动关闭这些动画。

操作：轮到你时点手牌即可打出。可以吃、碰、杠、胡时，牌桌中央会弹出大按钮面板，
**每个选项直接画出成型后的牌组**（碰画三张、杠画四张、吃画整副顺子，被吃的那张描金框）；
一张牌有多种吃法时每种吃法各一个按钮，点哪个就吃哪个。不想操作点「过」。

手牌下方的提示条会实时显示：未听牌时是「距听牌 N 张」；已听牌时是
**「打出 X → 听 A 18番 · B 差3番」**，直接告诉你打哪张、听什么、每张和了多少番、够不够起胡番。

> **为什么有时候和牌了却没有「胡」按钮？**
> 国标要求达到起胡番才能宣布和牌。这种情况下会显示一个**灰色的「胡」按钮标明「差 N 番」**，
> 并提示「这张能和牌，但番数不够」——不是 bug，是规则。把起胡番设成 0 番即可随意和牌。
